...
function DailyRecordPage({ back, home, records, setRecords, goto }) {
  ...
  const addMedia = () => {
    ...
    const emojis = ['📷', '🎨', '🎵', '📝', '🌿', '🌸'];
    setMedia([...media, { name: '新素材', emoji: emojis[media.length % emojis.length] }]);
  };
          ...
          <div className="grid grid-cols-3 gap-2">
            {media.map((m, i) => (
              <div key={i} className="aspect-square rounded-xl flex flex-col items-center justify-center relative"
                ...
                }} className="text-[9px] text-center text-white outline-none bg-transparent w-full mt-1"
                  style={{ borderBottom: '1px solid rgba(255,255,255,0.2)' }} />
                <button onClick={() => setMedia(media.filter((_, x) => x !== i))} className="absolute top-1 right-1">
                  <X size={10} color="white" />
                </button>
              </div>
            ))}
            {media.length < 3 && (
              <button onClick={addMedia} className="aspect-square rounded-xl flex flex-col items-center justify-center"
...
function FreeSharePage({ back, home, diaries, setDiaries }) {
  ...
  const save = () => {
    if (!text.trim()) return;
    setDiaries([...diaries, { id: 'd' + Date.now(), date: new Date().toISOString().slice(0, 10), text, bg, stickers }]);
    setText('');
    setStickers([]);
    home();
  };
    ...
    <div className="min-h-full pb-6">
      ...
      <div className="px-5">
        ...
        <div className="relative rounded-2xl p-4 mb-3" style={{ ...bgStyles[bg], minHeight: 280 }}>
          {stickers.map((s, i) => (
            <div key={i} className="absolute text-2xl" style={{ left: `${s.x}%`, top: `${s.y}%` }}>{s.emoji}</div>
          ))}
        ...
        <div className="grid grid-cols-6 gap-2 mb-3">
          {[
            { icon: Camera, label: '相機' },
            { icon: ImageIcon, label: '相簿' },
            { icon: FileText, label: '檔案' },
            { icon: Mic, label: '錄音' },
            { icon: Brush, label: '畫筆' },
            { icon: Sticker, label: '貼紙' },
          ].map((t, i) => {
            const I = t.icon;
            return (
              <button key={i} onClick={() => {
                if (t.label === '貼紙') addSticker(['🌿', '✨', '🌙', '⭐', '🍃', '🌸'][stickers.length % 6]);
            ...
            <div className="space-y-2 mt-2">
              {diaries.slice(-3).reverse().map(d => (
                <div key={d.id} className="rounded-xl p-3 text-xs"
                  ...
                  <div className="text-[10px] mb-1" style={{ color: C.accent }}>{d.date}</div>
